...
<head>
    ...
    <title>@yield('title', 'Curema') - CRM</title>
...
<body class="bg-curema-bg text-curema-ink antialiased"
    ...
    <aside class="hidden lg:flex flex-col fixed left-0 top-0 h-screen w-[250px] bg-curema-card border-r border-curema-border p-5 z-30">
        <a href="{{ route('crm.dashboard') }}" class="flex items-center gap-2 px-1 pb-6 mb-2 border-b border-curema-border">
            <div class="w-9 h-9 rounded-xl bg-curema-purplesoft flex items-center justify-center text-lg">🐱</div>
            <span class="text-xl font-extrabold tracking-tight">Curema</span>
        </a>
    ...
    @include('crm.partials.modals.export-modal')
    @include('crm.partials.modals.settings-modal')
    @include('crm.partials.modals.add-customer-modal')
    <script>
